                  ...
                  >
                    ...
                    >
                      ...
                      <v-list-item prepend-icon="mdi-briefcase">
                        ...
                        <v-list-item-subtitle>{{ selectedMember.currentProject }}</v-list-item-subtitle>
                        <template #append>
                          <v-btn
                            icon
                            variant="text"
    ...
    setup() {
      ...
      const messages = ref([
        ...
        // Chat 3 messages
        {
          ...
          timestamp: new Date(2025, 3, 14, 14, 0.5)
        },
        {
          chatId: 3,
          senderId: 1,
      ...
      // Fix the octal literal error by changing 05 to 5
      const fixOctalTimestamp = () => {
        const octalMessageIndex = messages.value.findIndex(
          msg => msg.chatId === 3 && msg.senderId === 3
        );
        if (octalMessageIndex !== -1) {
          messages.value[octalMessageIndex].timestamp = new Date(2025, 3, 14, 14, 5);
        }
      };
...
.v-chip {
  /* Add these styles to make skills more visible */
  background-color: #e0f2f1 !important; /* Light teal background */
  color: #00897b !important; /* Darker teal text */
  margin-right: 8px;
  margin-bottom: 8px;
  font-weight: 500;
}
...
/* Specific chat list styling from image */
.chat-list-card {
  border-radius: 8px;
  overflow: hidden;
  height: 100%;
...
/* Message styling for contacts list like in image */
.contact-item .v-list-item__title {
  font-size: 14px;
  font-weight: 500;
}

.contact-item .v-list-item__subtitle {
  color: #757575;
  font-size: 12px;
}
...
/* Custom search styling like in image */
.search-field {
  background-color: rgba(0, 0, 0, 0.03);
  border-radius: 20px;
  padding: 0 10px;
...
/* Left-side navigation matching image */
.left-nav .v-list-item {
  justify-content: center;
  min-height: 40px;
  width: 40px;
  margin: 10px auto;
...
.chat-item .v-list-item__content {
  min-height: 0;
}

.chat-item .v-list-item__title {
  font-size: 14px;
  font-weight: 500;
  line-height: 1.4;
}

.chat-item .v-list-item__subtitle {
  font-size: 12px;
  line-height: 1.2;
}
