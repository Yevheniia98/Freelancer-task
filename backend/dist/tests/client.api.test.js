...
describe('Client API', () => {
    ...
    afterEach(async () => {
        if (createdClientIds.length > 0) {
            await client_entity_1.ClientEntity.deleteMany({ _id: { $in: createdClientIds } });
            createdClientIds = [];
        }
    });
    ...
    describe('GET /api/clients', () => {
        ...
        it('should sort clients by totalEarned descending', async () => {
            ...
            for (let i = 1; i < response.body.data.length; i++) {
                expect(response.body.data[i - 1].totalEarned).toBeGreaterThanOrEqual(response.body.data[i].totalEarned);
            }
        });
    });
    ...
    describe('PUT /api/clients/:id', () => {
        ...
        it('should reject update with duplicate email', async () => {
            ...
            const anotherResponse = await (0, supertest_1.default)(app)
                .post('/api/clients')
                .send({
                name: 'Another Client',
    ...
    describe('PATCH /api/clients/bulk/earnings', () => {
        ...
        it('should update earnings for multiple clients', async () => {
            ...
            // Verify the updates
            const client1 = await (0, supertest_1.default)(app).get(`/api/clients/${clientIds[0]}`);
            const client2 = await (0, supertest_1.default)(app).get(`/api/clients/${clientIds[1]}`);
