...
exports.ProjectInviteController = void 0;
const crypto_1 = __importDefault(require("crypto"));
const project_invite_model_1 = require("../models/project-invite.model");
...
class ProjectInviteController {
    constructor() {
         ...
         */
        this.createInvite = async (req, res) => {
            try {
                ...
                const existingInvite = await project_invite_model_1.ProjectInvite.findOne({
                    projectId,
                ...
                const invite = await project_invite_model_1.ProjectInvite.create({
                    projectId,
                    email: email.toLowerCase(),
                    role,
                    token,
        ...
        this.acceptInvite = async (req, res) => {
            try {
                ...
                const invite = await project_invite_model_1.ProjectInvite.findOne({ token: token });
                if (!invite) {
                    res.status(404).json({
                        success: false,
                ...
                //   return;
                // }
                const project = await project_entity_1.ProjectEntity.findById(invite.projectId);
                if (!project) {
                    res.status(404).json({
                        success: false,
                        message: 'Project not found'
                ...
                if (project.teamMembers) {
                    project.teamMembers = project.teamMembers.filter(m => m.email.toLowerCase() !== invite.email.toLowerCase() || m.status !== 'pending');
                }
                else {
                    project.teamMembers = [];
                }
     ...
     */
    async sendInviteEmail(email, inviterName, projectName, role, inviteLink) {
...
exports.ProjectInviteController = ProjectInviteController;
