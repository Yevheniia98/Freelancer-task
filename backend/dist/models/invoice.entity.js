...
const InvoiceEntitySchema = new mongoose_1.Schema({
    clientId: {
        type: String,
...
InvoiceEntitySchema.virtual('formattedAmount').get(function () {
    return new Intl.NumberFormat('en-US', {
        style: 'currency',
        currency: this.currency
    }).format(this.amount);
});
InvoiceEntitySchema.virtual('formattedTotal').get(function () {
    return new Intl.NumberFormat('en-US', {
        style: 'currency',
        currency: this.currency
    }).format(this.total);
});
InvoiceEntitySchema.virtual('formattedSubtotal').get(function () {
    return new Intl.NumberFormat('en-US', {
        style: 'currency',
        currency: this.currency
...
InvoiceEntitySchema.virtual('daysOverdue').get(function () {
...
InvoiceEntitySchema.virtual('paymentStatus').get(function () {
    if (this.status === 'paid')
        return 'paid';
...
InvoiceEntitySchema.virtual('remainingAmount').get(function () {
    return this.total - (this.paidAmount || 0);
});
// Pre-save middleware
InvoiceEntitySchema.pre('save', function (next) {
...
// Instance methods
InvoiceEntitySchema.methods.markAsPaid = function (amount, method, reference) {
    this.status = 'paid';
    this.paidDate = new Date();
...
// Static methods
InvoiceEntitySchema.statics.findByClient = function (clientId) {
    return this.find({ clientId }).sort({ issueDate: -1 });
};
InvoiceEntitySchema.statics.findByStatus = function (status) {
...
InvoiceEntitySchema.statics.getTotalRevenue = function (clientId) {
...
exports.InvoiceEntity = mongoose_1.default.model('InvoiceEntity', InvoiceEntitySchema);
