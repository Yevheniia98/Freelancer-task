...
// Deal entity for managing sales pipeline
export interface IDealEntity extends Document {
  id: string;
  clientId: string;
  title: string;
  description?: string;
  ...
  // Instance methods
  moveToStage(stage: string, probability?: number): Promise<IDealEntity>;
  addActivity(activity: any): Promise<IDealEntity>;
  addNote(note: string): Promise<IDealEntity>;
  setNextAction(action: string, dueDate: Date): Promise<IDealEntity>;
  completeNextAction(): Promise<IDealEntity>;
  closeWon(): Promise<IDealEntity>;
  closeLost(reason?: string): Promise<IDealEntity>;
}

// Static methods interface
export interface IDealModel extends Model<IDealEntity> {
  findByClient(clientId: string): Promise<IDealEntity[]>;
  findByStage(stage: string): Promise<IDealEntity[]>;
  findActiveDeals(): Promise<IDealEntity[]>;
  findOverdueDeals(): Promise<IDealEntity[]>;
  getPipelineValue(): Promise<any[]>;
  getConversionStats(): Promise<any[]>;
}

const DealEntitySchema = new Schema<IDealEntity>({
  clientId: {
    type: String,
...
DealEntitySchema.virtual('formattedValue').get(function(this: IDealEntity) {
...
DealEntitySchema.virtual('weightedValue').get(function(this: IDealEntity) {
  return (this.value * this.probability) / 100;
});

// Virtual for formatted weighted value
DealEntitySchema.virtual('formattedWeightedValue').get(function(this: IDealEntity) {
...
DealEntitySchema.virtual('stageColor').get(function(this: IDealEntity) {
  const colors = {
    lead: '#6B7280',
    contacted: '#3B82F6',
    qualified: '#8B5CF6',
...
DealEntitySchema.virtual('stageDisplayName').get(function(this: IDealEntity) {
  const names = {
    lead: 'Lead',
    contacted: 'Contacted',
    qualified: 'Qualified',
...
DealEntitySchema.virtual('priorityColor').get(function(this: IDealEntity) {
  const colors = {
    low: '#10B981',
    medium: '#F59E0B',
    high: '#EF4444',
...
DealEntitySchema.virtual('daysToClose').get(function(this: IDealEntity) {
...
DealEntitySchema.virtual('dealAge').get(function(this: IDealEntity) {
  const today = new Date();
...
DealEntitySchema.virtual('isOverdue').get(function(this: IDealEntity) {
...
// Pre-save middleware
DealEntitySchema.pre('save', function(this: IDealEntity, next) {
  // Normalize tags
  if (this.tags) {
...
// Instance methods
DealEntitySchema.methods.moveToStage = function(stage: string, probability?: number) {
...
DealEntitySchema.methods.addActivity = function(activity: any) {
...
DealEntitySchema.methods.addNote = function(note: string) {
  this.notes = this.notes || [];
  this.notes.push(note);
  return this.save();
};

DealEntitySchema.methods.setNextAction = function(action: string, dueDate: Date) {
  this.nextAction = {
    action,
    dueDate,
    completed: false
...
DealEntitySchema.methods.closeLost = function(reason?: string) {
  if (reason) {
...
// Static methods
DealEntitySchema.statics.findByClient = function(clientId: string) {
  return this.find({ clientId }).sort({ updatedAt: -1 });
};

DealEntitySchema.statics.findByStage = function(stage: string) {
...
export const DealEntity = mongoose.model<IDealEntity, IDealModel>('DealEntity', DealEntitySchema);
