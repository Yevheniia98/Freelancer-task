...
import { CalendarEventEntity, EventType, ReminderType } from '../models/calendar-event.entity';
...
describe('Calendar Event API', () => {
  ...
  afterEach(async () => {
    if (createdEventIds.length > 0) {
      await CalendarEventEntity.deleteMany({ _id: { $in: createdEventIds } });
    }
  });
  ...
  describe('GET /api/calendar-events', () => {
    beforeEach(async () => {
      ...
      for (const eventData of events) {
        const event = new CalendarEventEntity(eventData);
        await event.save();
    ...
    it('should filter events by type', async () => {
      const response = await request(app)
        .get(`/api/calendar-events?eventType=${EventType.MEETING}`)
        .expect(200);

      expect(response.body.success).toBe(true);
      expect(response.body.data.length).toBe(1);
      expect(response.body.data[0].eventType).toBe(EventType.MEETING);
    });
    ...
    it('should sort events by date', async () => {
      ...
      const dates = response.body.data.map((event: any) => new Date(event.date));
  ...
  describe('DELETE /api/calendar-events/:id', () => {
    ...
    it('should delete calendar event by valid ID', async () => {
      ...
      // Verify deletion
      const deletedEvent = await CalendarEventEntity.findById(eventId);
  ...
  describe('PATCH /api/calendar-events/bulk/status', () => {
    ...
    it('should bulk update event status', async () => {
      ...
      for (const eventId of eventIds) {
        const event = await CalendarEventEntity.findById(eventId);
