...
class FinanceService {
  ...
  async getFinancialSummary(userId: string): Promise<FinancialSummary> {
    try {
      ...
      for (const connection of connections) {
        const platformTransactions = allTransactions.filter(
          (t) => t.platform === connection.platform
        );
        ...
        const activeProjectIds = new Set(
          platformTransactions
            .filter(
              (t) => t.projectId && t.transactionDate >= thirtyDaysAgo
            )
            .map((t) => t.projectId)
        );
      ...
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      const allActiveProjectIds = new Set(
        allTransactions
          .filter(
            (t) => t.projectId && t.transactionDate >= thirtyDaysAgo
          )
   ...
   */
  async syncPlatformEarnings(
    userId: string,
    platform: string
  ): Promise<{ success: boolean; message: string; transactionsAdded: number }> {
    try {
   ...
   */
  private async syncUpworkEarnings(
    userId: string,
    connection: any
  ): Promise<number> {
    try {
      ...
      const response = await axios.get('https://www.upwork.com/api/hr/v2/teams/owned/earnings', {
        headers: {
      ...
      let transactionsAdded = 0;
      const earnings = (response.data as any)?.table?.rows || (response.data as any)?.earnings || [];
   ...
   */
  private async refreshUpworkToken(connection: any): Promise<void> {
    try {
      const response = await axios.post('https://www.upwork.com/api/v3/oauth2/token', {
   ...
   */
  private async syncFreelancerEarnings(
    userId: string,
    connection: any
  ): Promise<number> {
    try {
      // Freelancer.com API documentation: https://developers.freelancer.com/
      const response = await axios.get('https://www.freelancer.com/api/projects/0.1/users/self/transactions/', {
        headers: {
      ...
      let transactionsAdded = 0;
      const transactions = (response.data as any)?.result?.transactions || [];
        ...
        try {
          ...
          if (projectId) {
            try {
              ...
              projectName = (projectResponse.data as any)?.result?.title || projectName;
            } catch (error) {
   ...
   */
  private async syncFiverrEarnings(
    userId: string,
    connection: any
  ): Promise<number> {
    try {
      ...
      const response = await axios.get('https://api.fiverr.com/v1/seller/orders', {
        headers: {
          'Authorization': `Bearer ${connection.accessToken}`,
   ...
   */
  async createDemoTransactions(userId: string): Promise<void> {
