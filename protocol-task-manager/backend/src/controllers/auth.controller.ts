...
export class AuthController {
  ...
  public register = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await this.securityMonitor.logEvent(SecurityEventType.LOGIN_SUCCESS, {
      ...
      await this.securityMonitor.logEvent(SecurityEventType.LOGIN_FAILED, {
        ipAddress: req.ip || req.connection.remoteAddress || 'unknown',
        userAgent: req.get('User-Agent'),
        severity: 'medium',
        metadata: {
  ...
  public login = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Track login attempt
      await this.securityMonitor.trackLoginAttempt(
        email.toLowerCase(),
      ...
      if (result.user) {
        await this.securityMonitor.checkSuspiciousLocation(
          String(result.user.id),
          req.ip || req.connection.remoteAddress || 'unknown'
        );

        // Track session
        await this.securityMonitor.trackSession(
          String(result.user.id),
          req.ip || req.connection.remoteAddress || 'unknown',
          'start'
        );
