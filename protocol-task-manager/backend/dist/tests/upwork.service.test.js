...
const upwork_service_1 = require("../services/upwork.service");
...
describe('UpworkService', () => {
    ...
    beforeEach(() => {
        ...
        upworkService = new upwork_service_1.UpworkService(mockPlatform);
    });
    ...
    describe('fetchProjects', () => {
        ...
        beforeEach(() => {
            ...
            upworkService.httpClient = mockHttpClient;
        });
        it('should fetch projects successfully', async () => {
            const mockHttpClient = upworkService.httpClient;
            mockHttpClient.get
    ...
    describe('OAuth handling', () => {
        it('should handle OAuth callback successfully', async () => {
            ...
            expect(result).toBe(true);
            expect(mockPlatform.accessToken).toBeDefined();
            expect(mockPlatform.refreshToken).toBeDefined();
            expect(mockPlatform.isActive).toBe(true);
        });
        it('should refresh access token', async () => {
            const oldToken = mockPlatform.accessToken;
            const result = await upworkService.refreshAccessToken();
            expect(result).toBe(true);
            expect(mockPlatform.accessToken).not.toBe(oldToken);
        });
        it('should fail to refresh without refresh token', async () => {
            mockPlatform.refreshToken = '';
            const result = await upworkService.refreshAccessToken();
            expect(result).toBe(false);
        });
    });
